          ...
          <div className="p-4 sm:p-5 sm:pb-3 border-b border-gray-100 flex flex-col flex-shrink-0 gap-4 z-10 relative">
            ...
            <div className="flex bg-gray-100/80 p-1.5 rounded-[1.2rem]">
              <button
                ...
                className={`flex-1 py-2 text-sm font-bold rounded-xl transition-all duration-300 ${activeTab === 'active' ? 'bg-white text-black shadow-sm' : 'text-gray-500 hover:text-black'}`}
              >
                Active
              </button>
              <button
                onClick={() => setActiveTab('completed')}
                className={`flex-1 py-2 text-sm font-bold rounded-xl transition-all duration-300 ${activeTab === 'completed' ? 'bg-white text-black shadow-sm' : 'text-gray-500 hover:text-black'}`}
              >
                Completed
              </button>
            </div>
            <div className="relative group">
              ...
              <input
                ...
                className="w-full bg-gray-50 border border-gray-100/50 rounded-2xl pl-12 pr-4 py-3.5 text-sm focus:ring-2 focus:ring-[#d9ff00] focus:bg-white transition-all font-medium placeholder:text-gray-400"
              />
            </div>
          </div>
                ...
                {filteredChats.map((chat) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-4 sm:p-5 hover:bg-gray-50 transition-all flex items-start gap-4 border-l-4 ${isActive ? 'bg-gray-50 border-black' : 'border-transparent'}`}
                    >
                      <div className="w-14 h-14 rounded-full bg-gray-100 flex-shrink-0 overflow-hidden relative border border-gray-200">
                        {otherUser?.photoURL ? (
                        ...
                        {(chat.unreadCount as any)?.[user?.uid || ''] > 0 && (
                          <div className="absolute top-0 right-0 w-3.5 h-3.5 bg-red-500 rounded-full border-2 border-white shadow-sm"></div>
                        )}
                      </div>
                      <div className="flex-1 min-w-0 pt-0.5">
                        <div className="flex justify-between items-baseline mb-1">
                          <h3 className={`truncate pr-2 tracking-tight ${isActive ? 'font-black text-gray-900' : 'font-bold text-gray-900'}`}>{otherUser?.name || 'Unknown User'}</h3>
                          {chat.lastMessageAt && (
                            <div className="flex flex-col items-end gap-1 flex-shrink-0">
                              <span className={`text-[10px] font-bold uppercase tracking-wider ${((chat.unreadCount as any)?.[user?.uid || ''] > 0) ? 'text-red-500' : 'text-gray-400'}`}>
                        ...
                        {chat.productTitle && (
                          <p className="text-[10px] bg-black text-[#d9ff00] font-black uppercase tracking-widest truncate mb-2 flex items-center gap-1.5 px-2 py-0.5 rounded-md inline-flex max-w-[90%]">
                            <ShoppingBag className="w-3 h-3 flex-shrink-0" /> <span className="truncate">{chat.productTitle}</span>
                          </p>
                        )}
                        <p className={`text-sm truncate ${((chat.unreadCount as any)?.[user?.uid || ''] > 0) ? 'font-bold text-gray-900' : 'text-gray-500 font-medium'}`}>{chat.lastMessage || 'No messages yet'}</p>
                      </div>
                    </button>
                  );
                })}
          ...
          {activeChatId && otherParticipant ? (
            <>
              ...
              <div className="h-20 lg:h-24 px-4 sm:px-6 bg-white/90 backdrop-blur-xl border-b border-gray-100 flex items-center gap-4 flex-shrink-0 sticky top-0 z-20 pt-[env(safe-area-inset-top,0px)]">
                ...
                <div className="w-12 h-12 rounded-full bg-gray-100 overflow-hidden relative flex-shrink-0 border border-gray-200">
                ...
                <div className="flex-1 min-w-0">
                  <h2 className="text-lg font-black text-gray-900 truncate tracking-tight">{otherParticipant.name}</h2>
                  <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">{otherParticipant.role}</p>
                </div>
              ...
              {activeOrder && (
                <div className="bg-white border-b border-gray-100 p-4 sm:p-5 shadow-sm z-10 flex-shrink-0">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center">
                      <ShieldCheck className="w-4 h-4 text-indigo-600" />
                    </div>
                    <h3 className="font-bold text-gray-900 tracking-tight">Escrow Protected Order</h3>
                    <span className="text-[10px] font-black px-2.5 py-1 rounded-md bg-indigo-50 text-indigo-700 ml-auto uppercase tracking-widest border border-indigo-100">
              ...
              <div className="flex-1 overflow-y-auto min-h-0 p-4 sm:p-6 space-y-6 relative">
                ...
                {messages.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-gray-400 space-y-4 relative z-10">
                    <div className="w-20 h-20 bg-white rounded-[2rem] flex items-center justify-center shadow-sm border border-gray-100">
                      <MessageSquare className="w-8 h-8 text-gray-400" />
                    </div>
                    <p className="font-medium text-gray-500">Say hello!</p>
                  </div>
                ) : (
                      ...
                      <div key={msg.id} className="space-y-3 relative z-10">
                        {showTime && msg.createdAt && (
                          <div className="flex justify-center my-6">
                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-4 py-1.5 rounded-full border border-gray-100 bg-white shadow-sm">
                        ...
                        <div className={`flex ${msg.isSystem ? 'justify-center' : isMe ? 'justify-end' : 'justify-start'}`}>
                          {msg.isSystem ? (
                            <div className="max-w-[85%] bg-blue-50 text-blue-800 border border-blue-100 rounded-3xl p-5 text-center mx-auto my-4 shadow-sm flex flex-col items-center gap-3">
                            ...
                            <div className={`max-w-[80%] sm:max-w-[65%] p-4 sm:p-5 shadow-sm group relative ${
                              isMe 
                                ? 'bg-black text-white rounded-[1.5rem] rounded-br-sm border border-gray-800 ml-auto' 
                                : 'bg-white border border-gray-100 text-gray-900 rounded-[1.5rem] rounded-bl-sm mr-auto'
                            }`}>
                              <p className="text-[15px] leading-relaxed whitespace-pre-wrap font-medium">{msg.text}</p>
                              {msg.createdAt && (
                                <span className={`text-[9px] font-bold uppercase tracking-wider opacity-0 group-hover:opacity-100 transition-opacity absolute bottom-1.5 ${isMe ? 'right-4 text-gray-400' : 'left-4 text-gray-400'}`}>
                                  {new Date(msg.createdAt.toDate()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                </span>
                              )}
                            </div>
              ...
              <div className="p-4 sm:p-6 bg-white border-t border-gray-100 flex-shrink-0 z-10 relative mt-auto pb-[max(1rem,env(safe-area-inset-bottom,1rem))]">
                {activeOrder?.status === 'completed' ? (
                  <div className="bg-gray-50 rounded-[2rem] p-6 text-center shadow-inner border border-gray-100">
                    ...
                    <h4 className="font-black text-gray-900 mb-1 tracking-tight text-lg">Order Completed</h4>
                    <p className="text-sm text-gray-500 mb-4 font-medium">This chat is now read-only.</p>
                  ...
                  <form onSubmit={handleSendMessage} className="flex relative">
                    <input
                      ...
                      className="w-full bg-gray-50 border border-gray-100 rounded-full pl-6 pr-16 py-4 text-[15px] focus:ring-2 focus:ring-[#d9ff00] focus:bg-white transition-all font-medium placeholder:text-gray-400 shadow-inner"
                    />
                    <button
                      type="submit"
                      disabled={!newMessage.trim()}
                      className="absolute right-1.5 top-1/2 -translate-y-1/2 w-11 h-11 bg-black text-[#d9ff00] rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:hover:scale-100 border border-gray-800 shadow-md"
                    >
            ...
            <div className="h-full flex flex-col items-center justify-center text-gray-400 space-y-6 relative overflow-hidden bg-white">
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-[#d9ff00] rounded-full blur-[150px] opacity-10 pointer-events-none"></div>
              <div className="w-24 h-24 bg-gray-50 rounded-[2.5rem] flex items-center justify-center shadow-sm border border-gray-100 relative z-10">
                <MessageSquare className="w-10 h-10 text-gray-300" />
              </div>
              <div className="text-center relative z-10">
                <h3 className="text-3xl font-black tracking-tighter text-gray-900 mb-2">Your Messages</h3>
